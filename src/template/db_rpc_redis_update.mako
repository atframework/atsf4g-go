...
% if cas_enabled:
			ctx.GetApp().GetLogger(2).LogDebug("EvalSha HSet ${message_name} Send", "Seq", awaitOption.Sequence, "index", index, "currentCASVersion", OldCASVersion, "data", table)
			cmdResult, redisError := instance.EvalSha(ctx.GetContext(), dispatcher.GetCASLuaSHA(), []string{index}, redisData).Result()
% else:
			ctx.GetApp().GetLogger(2).LogDebug("HSet ${message_name} Send", "Seq", awaitOption.Sequence, "index", index, "data", table)
...
% endif
			if redisError != nil {
% if cas_enabled:
				ctx.GetApp().GetLogger(2).LogError("EvalSha HSet ${message_name} Recv Error", "Seq", awaitOption.Sequence, "currentCASVersion", *currentCASVersion, "redisError", redisError)
% else:
				ctx.GetApp().GetLogger(2).LogError("HSet ${message_name} Recv Error", "Seq", awaitOption.Sequence, "redisError", redisError)
% endif
...
% if cas_enabled:
			ctx.GetApp().GetLogger(2).LogDebug("EvalSha HSet ${message_name} Recv", "Seq", awaitOption.Sequence, "realVersion", realVersion)
			resumeData.PrivateData = &realVersion
			resumeData.Result = cd.CreateRpcResultOk()
% else:
			ctx.GetApp().GetLogger(2).LogDebug("HSet ${message_name} Recv", "Seq", awaitOption.Sequence)
			resumeData.Result = cd.CreateRpcResultOk()
% endif
			resumeError := cd.ResumeTaskAction(ctx, currentAction, resumeData)
			if resumeError != nil {
...
% if cas_enabled:
	...
	*currentCASVersion = *realVersion
	if OldCASVersion != 0 && OldCASVersion+1 != *currentCASVersion {
		ctx.GetApp().GetLogger(2).LogInfo("EvalSha HSet ${message_name} CAS Check Failed", "Seq",
