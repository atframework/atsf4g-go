...
type TaskActionMailReceiveAttachmentsAll struct {
	user_controller.TaskActionCSBase[*service_protocol.CSMailReceiveAttachmentsAllReq, *service_protocol.SCMailReceiveAttachmentsAllRsp]
}
...
func (t *TaskActionMailReceiveAttachmentsAll) Run(_startData *component_dispatcher.DispatcherStartData) error {
	...
	mailOperateResult, rpcResult := mailMgr.ReceiveMailAttachmentsAll(t.GetRpcContext(), requestBody.NeedRemove)
	...
	for _, result := range mailOperateResult {
