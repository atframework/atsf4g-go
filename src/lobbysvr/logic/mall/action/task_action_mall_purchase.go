...
type TaskActionMallPurchase struct {
	user_controller.TaskActionCSBase[*service_protocol.CSMallPurchaseReq, *service_protocol.SCMallPurchaseRsp]
}
...
func (t *TaskActionMallPurchase) Run(_startData *component_dispatcher.DispatcherStartData) error {
	...
	t.SetResponseCode(manager.MallPurchase(t.GetRpcContext(), request_body.GetReqs(), response_body))
