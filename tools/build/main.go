...
func buildService(projectBaseDir string, buildPath string, sourcePath string, outputPath string) error {
	...
	if outputBinRelPath, err := filepath.Rel(workDir, outputBinPath); err == nil {
		project_settings.FmtColorPrintCyan("Run: go build -o %s", path.Join(outputBinRelPath, project_settings.ServiceBinName(path.Base(sourcePath))))
		cmd = exec.Command("go", "build", "-o", path.Join(outputBinRelPath, project_settings.ServiceBinName(path.Base(sourcePath))))
	} else {
		project_settings.FmtColorPrintCyan("Run: go build -o %s", path.Join(outputBinPath, project_settings.ServiceBinName(path.Base(sourcePath))))
	...
	err := cmd.Run()
	if err != nil {
		project_settings.FmtColorPrintRed("Run build Error %s", err)
		return err
	}

	project_settings.FmtColorPrintGreen("Run build %s to %s success", sourcePath, outputBinPath)
	return nil
}
...
func main() {
	...
	// 1.generate
	{
		...
		err := cmd.Run()
		if err != nil {
			project_settings.FmtColorPrintRed("Run generate Error %s", err)
			os.Exit(1)
		}
		project_settings.FmtColorPrintGreen("Run generate success")
	}
	...
	project_settings.FmtColorPrintGreen("Run Go Mod success")
	...
	project_settings.FmtColorPrintGreen("Build success")
