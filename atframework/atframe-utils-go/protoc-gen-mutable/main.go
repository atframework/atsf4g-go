...
func generateFile(plugin *protogen.Plugin, f *protogen.File) {
	...
	if len(f.Messages) != 0 {
		...
		g.P("import lu \"github.com/atframework/atframe-utils-go/lang_utility\"")
...
func generateMutableForMessage(f *protogen.File, g *protogen.GeneratedFile, msg *protogen.Message) {
	...
	g.P(fmt.Sprintf(`func (_ *%s) GetTypeID() lu.TypeID {`, msg.GoIdent.GoName))
	g.P(fmt.Sprintf(`  return lu.GetTypeIDOf[%s]()`, msg.GoIdent.GoName))
	g.P(`}`)
	g.P()
	...
	for _, oneof := range msg.Oneofs {
		...
		g.P(fmt.Sprintf(`	GetTypeID%s() lu.TypeID`, fullFieldName))
		...
		g.P("// ===== GetOneofTypeID methods for ", msg.GoIdent.GoName, " Oneof ", oneofName, " ===== Oneof =====")
		g.P(fmt.Sprintf(`func (m *%s) Get%sTypeID() lu.TypeID {`, msg.GoIdent.GoName, oneofName))
		g.P(`  if m == nil {`)
		g.P(`    return 0`)
		g.P(`  }`)
		g.P(fmt.Sprintf(`	v, ok := m.%s.(get%s)`, oneofName, fullFieldName))
		...
		g.P(`		return 0`)
		g.P(`  }`)
		g.P(fmt.Sprintf(`	return v.GetTypeID%s()`, fullFieldName))
		g.P(`}`)
		g.P()
	}
	...
	for _, field := range msg.Fields {
		...
		case field.Oneof != nil:
			...
			g.P(fmt.Sprintf(`func GetTypeID%s() lu.TypeID {`, fullFieldName))
			g.P(fmt.Sprintf(`	return lu.GetTypeIDOf[%s]()`, fullFieldName))
			g.P(`}`)
			g.P(fmt.Sprintf(`func (_ *%s) GetTypeID() lu.TypeID {`, fullFieldName))
			g.P(fmt.Sprintf(`  return lu.GetTypeIDOf[%s]()`, fullFieldName))
			g.P(`}`)
			g.P()
			...
			g.P(`}`)
			g.P(fmt.Sprintf(`func (m *%s) GetTypeID%s_%s() lu.TypeID {`, fullFieldName, msg.GoIdent.GoName, oneofName))
			g.P(fmt.Sprintf(`  return lu.GetTypeIDOf[%s]()`, fullFieldName))
			g.P(`}`)
...
func generateReadonlyForMessage(f *protogen.File, g *protogen.GeneratedFile, msg *protogen.Message) {
	...
	for _, oneof := range msg.Oneofs {
		...
		g.P(fmt.Sprintf("func (r *%s) Get%sTypeID() lu.TypeID {", roName, oneof.GoName))
		g.P("  if r == nil {")
		g.P("    return 0")
		g.P("  }")
		g.P(fmt.Sprintf("  if r.%s == nil {", oneofVar))
		g.P("    return 0")
		g.P("  }")
		g.P(fmt.Sprintf("  return r.%s.GetTypeID%s()", oneofVar, fullFieldName))
		g.P("}")
		g.P()
...
func generateReadonlyOneofInterface(g *protogen.GeneratedFile, msg *protogen.Message, oneof *protogen.Oneof) {
	...
	for _, field := range oneof.Fields {
		...
		g.P("}")
		g.P()
		g.P(fmt.Sprintf("func (r *%s) GetTypeID%s() lu.TypeID {", structName, fullFieldName))
		g.P(fmt.Sprintf("  return GetTypeID%s()", field.GoIdent.GoName))
		g.P("}")
		g.P()
